...
description: >-
  ...
  note found. Default auto-writes on a synthetic backfill branch and reports
  what was cached. Dry-run lists the exact write_knowledge payloads (title,
  content, repo, branch) without writing. Use when the user says "Please
  bootstrap my knowledge with Dosu", "bootstrap agent knowledge",
  "/bootstrap-agent-knowledge", "log to dosu knowledge", "mine my sessions into
  Dosu", "backfill branch notes from my agent logs", "save my agent logs to
  Dosu", or wants a one-shot pass over local histories.
---
...
4. Tell the user **what was cached** and the backfill branch
...
What to write / exclude: follow the write rules in
[references/miner-system-prompt.md](references/miner-system-prompt.md) — the
same contract the background miner uses. Two divergences for this interactive
flow (documented there): always use `BACKFILL_BRANCH` instead of
transcript-verified branches, and use `tags` instead of miner metadata.
...
- [ ] 4a. Default: write on BACKFILL_BRANCH (auto-promotes) → reply with cached titles
...
For each user turn that got an assistant conclusion passing the write rules in
[references/miner-system-prompt.md](references/miner-system-prompt.md), append
a payload. **Do not treat “I already wrote one note from this transcript” as
done.**
...
`transcript_id` is bookkeeping for the run — omit it from `write_knowledge`.
...
`{ "candidates": [ …payloads… ] }` so the default and dry-run paths share one
shape.

### Step 4a — Default: write + summary

For each payload, call MCP `write_knowledge` with `title` / `content` / `repo` /
`branch` / `tags` (omit `transcript_id`). Every write must use
`BACKFILL_BRANCH`. The server auto-enqueues notes-upflow for
`dosu/log-backfill/*` (same step as a PR merge) — no separate promote call.

Default user-facing reply shape:
...
## Extraction completeness (do not say "miss-mode" to the customer)
...
3. Bootstrap-only sessions are excluded from the default 50 by the parser; skip them if they appear.
4. If the Library already has the page (this run's `read_knowledge`), skip the write; status `already_in_library` is OK.

## Guardrails

- Default **writes** on `dosu/log-backfill/*` (server auto-promotes) and always
  reports the cached note titles and the backfill branch.
...
- User-facing output is always about **notes** (written or proposed),
  never raw prompts.

## Quick examples

- "Please bootstrap my knowledge with Dosu." → write on backfill branch (auto-promotes) + cached titles.
